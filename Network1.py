...
class FunctionJ(nn.Module):
    ...
    def forward(self, x):
        ...
        lck = torch.stack([self.center(x[0][k]) for k in range(N_C)])
        lqk = x[1]
        return torch.tensor([[self.distance(self.embedding(x), ki) for ki in lck] for x in lqk])
...
while True:
    ...
    optimizer.zero_grad()
    t = net(inp)
    j = torch.tensor(0., requires_grad=True).cuda()
    for a in range(N_C):
        j += (t[a][a] +
              torch.logsumexp(
                  torch.stack([torch.neg(t[a][b]) for b in range(N_C) if a != b]), 0)) / (N_C * N_Q)
    loss = j
    loss.backward()
    optimizer.step()
    ...
    if (time.time() - start) > TIME_LIMIT:
        torch.save(net, os.join('Net', '2019-7-27-01.pt'))
        break
